...
test('GitHub Pages 用 LIFF 画面は必要スクリプトと要素を持つ', async () => {
  ...
  assert.match(html, /<link rel="stylesheet" href="\.\/style\.css\?v=task-period-tabs-20260425" \/>/);
  assert.match(html, /<script src="\.\/app\.js\?v=task-period-tabs-20260425" defer><\/script>/);
...
test('GitHub Pages は管理者のテンプレート挿入イベントを当日のチェックリストへ即時反映する', async () => {
  ...
  assert.match(appJs, /if \(eventPayload\.type === 'template_insert'\) \{\s*applyTemplateInsertRealtimeEvent\(eventPayload\);\s*return;\s*\}/);
