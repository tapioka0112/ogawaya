...
test('GitHub Pages 用 LIFF 画面は必要スクリプトと要素を持つ', async () => {
  ...
  assert.match(html, /<script src="\.\/app\.js\?v=period-anchor-merge-20260511" defer><\/script>/);
...
test('GitHub Pages の check/uncheck は Firestore item と event を直接書き込む', async () => {
  ...
  assert.match(appJs, /function writeItemStatusToFirestore\(runItemId,\s*desiredStatus,\s*timeoutMs\)/);
  assert.match(appJs, /state\.api\.checkItem\(state\.idToken,\s*state\.accessToken,\s*runItemId\)/);
...
test('GitHub Pages の Firestore 同期はタイムアウト付きで実行する', async () => {
  ...
  assert.match(appJs, /writeItemStatusToFirestore\(runItemId,\s*desiredStatus,\s*ITEM_ACTION_REQUEST_TIMEOUT_MS\)/);
});
...
test('GitHub Pages は管理者のテンプレート挿入イベントを当日のチェックリストへ即時反映する', async () => {
  ...
  assert.match(appJs, /return !\(item\.templateItemId && existingTemplateItemIds\[item\.templateItemId\]\);/);
