...
test('GitHub Pages の GAS API 呼び出しは idToken と accessToken を POST body に載せる', async () => {
  const appJs = await readFile('pages/app.js', 'utf8');

  assert.match(appJs, /function buildLegacyBody\(idToken,\s*accessToken,\s*body\)/);
  assert.match(appJs, /requestBody\.authToken = idToken;/);
  assert.match(appJs, /requestBody\.accessToken = accessToken;/);
  assert.match(appJs, /requestBody\.liffId = liffId;/);
  assert.match(appJs, /liffId:\s*config\.liffId/);
  assert.match(appJs, /if\s*\(method === 'GET' && \(idToken \|\| accessToken\)\)\s*\{\s*actualMethod = 'POST';\s*query\._method = 'GET';/);
});
...
test('GitHub Pages の check/uncheck は Firestore 直接書き込み成功時にGASを待たずに確定する', async () => {
  const appJs = await readFile('pages/app.js', 'utf8');
...
test('GitHub Pages のGAS fallback同期はタイムアウト付きで実行する', async () => {
  const appJs = await readFile('pages/app.js', 'utf8');

  assert.match(appJs, /var ITEM_ACTION_REQUEST_TIMEOUT_MS = 8000;/);
  assert.match(appJs, /function syncItemStatusViaGas\(runItemId,\s*desiredStatus,\s*timeoutMs\)/);
  assert.match(appJs, /withTimeout\(\s*requestPromise,\s*timeoutMs,\s*'API 同期がタイムアウトしました'\s*\);/);
  assert.match(appJs, /writeRealtimeEvent\(optimisticItemForDispatch\)\.then/);
});
