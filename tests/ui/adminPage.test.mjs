...
test('管理者ページのテンプレートは期間専用で作成・挿入する', async () => {
  ...
  assert.match(js, /writeTemplateInsertEvent\(targetDate,\s*template\.id,\s*templatePeriod,\s*optimisticItems\)\.catch/);
  assert.match(js, /syncTemplateInsertViaGasInBackground\(targetDate,\s*template\.id,\s*templatePeriod,\s*optimisticItems,\s*0\);/);
});
...
test('管理者ページのテンプレート挿入は即時表示しFirestore通知後にGAS保存をバックグラウンド実行する', async () => {
  ...
  assert.match(js, /if \(!getRunIdForDate\(targetDate\)\) \{\s*await syncTemplateInsertViaGas\(targetDate,\s*template\.id,\s*templatePeriod,\s*optimisticItems\);/);
  assert.match(js, /writeTemplateInsertEvent\(targetDate,\s*template\.id,\s*templatePeriod,\s*optimisticItems\)\.catch/);
  assert.match(js, /syncTemplateInsertViaGasInBackground\(targetDate,\s*template\.id,\s*templatePeriod,\s*optimisticItems,\s*0\);/);
