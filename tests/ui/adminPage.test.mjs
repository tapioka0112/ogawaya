...
test('管理者ページのテンプレートは期間専用で作成・挿入する', async () => {
  ...
  assert.match(js, /period:\s*templatePeriod,\s*taskIds:\s*taskIds/);
  ...
  assert.match(js, /if \(!state\.config\.functionsApiBaseUrl\) \{\s*syncTemplateInsertViaGasInBackground\(targetDate,\s*template\.id,\s*templatePeriod,\s*optimisticItems,\s*0\);/);
});

test('管理者ページの GAS API 呼び出しは CORS preflight を避ける', async () => {
  const js = await readFile('pages/admin.js', 'utf8');

  assert.match(js, /function functionsApiRequest\(method,\s*path,\s*body,\s*options\)/);
  assert.match(js, /fetchOptions\.headers\['Content-Type'\] = 'application\/json';/);
...
test('管理者ページの管理データ取得と操作後反映は不要な全再取得を避ける', async () => {
  ...
  assert.match(js, /if \(response && Array\.isArray\(response\.items\) && response\.items\.length > 0\) \{\s*appendRunItemsForDate\(responseTargetDate,\s*response\.items\);/);
});
...
test('管理者ページのテンプレート挿入は即時表示しFirestore通知後にGAS保存をバックグラウンド実行する', async () => {
  ...
  assert.match(js, /function syncTemplateInsertViaGas\(targetDate,\s*templateId,\s*period,\s*items\)/);
  assert.match(js, /function syncTemplateInsertViaGasInBackground\(targetDate,\s*templateId,\s*period,\s*items,\s*attempt\)/);
  ...
  assert.match(js, /sourceUserId:\s*'admin:' \+ state\.storeId/);
  ...
  assert.match(js, /await writeTemplateInsertEvent\(targetDate,\s*template\.id,\s*templatePeriod,\s*optimisticItems\);/);
  assert.match(js, /if \(!state\.config\.functionsApiBaseUrl\) \{\s*syncTemplateInsertViaGasInBackground\(targetDate,\s*template\.id,\s*templatePeriod,\s*optimisticItems,\s*0\);/);
  assert.match(js, /function deleteRunItemViaGas\(targetDate,\s*runItemId\)/);
  assert.match(js, /function syncRunItemDeleteViaGasInBackground\(targetDate,\s*runItemId,\s*attempt\)/);
  assert.match(js, /if \(state\.config\.clientFirestoreWriteEnabled === true\) \{\s*await writeRunItemDeleteEvent\(state\.selectedDate,\s*runItemId\);[\s\S]*syncRunItemDeleteViaGasInBackground\(state\.selectedDate,\s*runItemId,\s*0\);/);
  assert.doesNotMatch(js, /if \(state\.config\.functionsApiBaseUrl\) \{\s*await writeRunItemDeleteEvent\(state\.selectedDate,\s*runItemId\);[\s\S]*\} else \{\s*await apiRequest\(\s*'DELETE'/);
