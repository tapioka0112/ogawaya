...
test('管理者ページの管理データ取得と操作後反映は不要な全再取得を避ける', async () => {
  ...
  assert.match(js, /if \(response\.item\) \{\s*appendCurrentRunItems\(\[response\.item\]\);/);
  assert.match(js, /if \(Array\.isArray\(response\.items\)\) \{\s*appendCurrentRunItems\(response\.items\);/);
});
