...
test('GET /api/checklists/today は Firestore snapshot を保存する', async () => {
  ...
  assert.equal(response.body.snapshotSync.status, 'ok');
  assert.equal(response.body.snapshotSync.responseCode, 200);
});
...
test('Firestore snapshot 保存失敗時もチェックリスト取得は成功する', async () => {
  ...
  assert.equal(response.body.snapshotSync.responseCode, 500);
  assert.match(response.body.snapshotSync.response, /firestore unavailable/);
});

test('Firestore snapshot client が非2xxを返した場合は成功扱いしない', async () => {
  const app = await createChecklistApp({
    snapshotClient: {
      writeTodaySnapshot() {
        return {
...
test('FIREBASE_PROJECT_ID 未設定時は既定の Firebase project に snapshot を保存する', async () => {
  ...
  assert.equal(response.statusCode, 200);
  assert.equal(firestoreWrites.length, 1);
  assert.match(firestoreWrites[0].url, /\/projects\/owagaya-fd93b\/databases\/\(default\)\/documents\//);
  assert.equal(response.body.snapshotSync.status, 'ok');
});
...
test('check 後に checked へ遷移し、チェック者は LINE 表示名で保存される', async () => {
  ...
  assert.equal(response.body.item.description, '券売機と入口を確認する');
