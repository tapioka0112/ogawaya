...
test('Spreadsheet 読み込みは period 追加前のヘッダーでも列ずれしない', async () => {
  const runItemsSheet = createSheet(
    ['id', 'run_id', 'template_item_id', 'title', 'sort_order', 'status', 'checked_by', 'checked_by_name', 'checked_at', 'updated_at'],
    [
  ...
  const runItems = repository.listTable('checklist_run_items');
  assert.equal(runItems[0].period, '');
  assert.equal(runtime.Ogawaya.normalizeTaskPeriod(runItems[0].period), 'daily');
  assert.equal(runItems[0].sort_order, '1');
...
test('updateRunItemWithLog は Spreadsheet 全量再書き込みせず対象行だけ更新する', async () => {
  ...
  assert.equal(runItemsSheet.getValues()[1][6], 'checked');
  assert.equal(logsSheet.getValues().length, 2);
  assert.equal(logsSheet.getValues()[1][0], 'log-001');
});

test('updateRunItem は Spreadsheet 全量再書き込みせず対象行だけ更新する', async () => {
  const cache = createScriptCache();
  const runItemsSheet = createSheet(
    ['id', 'run_id', 'template_item_id', 'title', 'period', 'sort_order', 'status', 'checked_by', 'checked_by_name', 'checked_at', 'updated_at'],
    [
  ...
  const repository = runtime.Ogawaya.createSpreadsheetRepository({ spreadsheetId: 'spreadsheet-001' });
  const updated = repository.updateRunItem('run-item-001', {
    status: 'checked',
    checked_by: 'user-001',
    checked_by_name: '田中LINE',
    checked_at: '2026-04-22T12:05:00Z',
...
test('updateRunItem は period 追加前ヘッダーを既存行ごと移行する', async () => {
  const cache = createScriptCache();
  const runItemsSheet = createSheet(
    ['id', 'run_id', 'template_item_id', 'title', 'sort_order', 'status', 'checked_by', 'checked_by_name', 'checked_at', 'updated_at'],
    [
  ...
  const repository = runtime.Ogawaya.createSpreadsheetRepository({ spreadsheetId: 'spreadsheet-001' });
  repository.updateRunItem('run-item-001', {
    status: 'checked',
    checked_by: 'user-001',
    checked_by_name: '田中LINE',
    checked_at: '2026-04-22T12:05:00Z',
  ...
  assert.deepEqual(runItemsSheet.getValues()[0], [
    'id',
    'run_id',
    'template_item_id',
    'title',
  ...
  assert.equal(runItemsSheet.getValues()[1][4], '');
  assert.equal(runItemsSheet.getValues()[1][5], '1');
  assert.equal(runItemsSheet.getValues()[1][6], 'checked');
