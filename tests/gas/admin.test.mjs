...
test('テンプレート新規作成・更新・項目 CRUD が成功する', async () => {
  ...
  assert.equal(createTemplate.body.template.period, 'daily');
...
test('管理者画面向け API でログイン後にタスク作成・挿入・テンプレ適用・削除できる', async () => {
  ...
  assert.equal(createTemplate.statusCode, 201);
  assert.equal(createTemplate.body.template.period, 'weekly');
...
test('管理者テンプレート一覧は挿入用にテンプレート項目を返す', async () => {
  ...
  assert.equal(dailyTemplate.period, 'daily');
...
test('管理者テンプレート一覧は既存テンプレートの closing_time 不正を補修して返す', async () => {
  const runtime = await loadGasRuntime({
    scriptProperties: {
      ADMIN_LOGIN_ID: 'admin-login',
      ADMIN_LOGIN_PASSWORD: 'admin-password'
  ...
  const seed = createBaseDataset();
  seed.checklist_templates[0].period = '';
  seed.checklist_templates[0].closing_time = 'invalid';
  const app = runtime.Ogawaya.createApplication({
    storage: runtime.Ogawaya.createArrayStorage(seed)
  });
  ...
  assert.equal(response.statusCode, 200);
  assert.equal(response.body.templates[0].closingTime, '00:00');
  assert.equal(app.repository.findTemplateById('tmpl-001').closing_time, '00:00');
  assert.equal(app.repository.findTemplateById('tmpl-001').period, 'daily');
});
...
test('管理者テンプレート作成は期間混在を拒否する', async () => {
  const app = await createAdminApp();
  const token = loginAsAdmin(app);

  const createWeeklyTask = app.handleApiRequest({
    method: 'POST',
    path: '/api/admin/tasks',
    query: { adminToken: token },
    body: {
  ...
  assert.equal(createWeeklyTask.statusCode, 201);
  ...
  const response = app.handleApiRequest({
    ...
    body: {
      name: '混在テンプレート',
      period: 'daily',
      taskIds: [createWeeklyTask.body.task.id]
    }
  });
...
test('管理者テンプレート一覧は既存混在テンプレートを期間別に自動分割する', async () => {
  const runtime = await loadGasRuntime({
    scriptProperties: {
      ADMIN_LOGIN_ID: 'admin-login',
      ADMIN_LOGIN_PASSWORD: 'admin-password'
  ...
  assert.equal(response.statusCode, 200);
  assert.equal(app.repository.findTemplateById('tmpl-001').is_active, 'false');
  assert.deepEqual(
    response.body.templates.map((template) => [template.id, template.period, template.itemCount]),
    [
      ['tmpl-001-daily', 'daily', 1],
      ['tmpl-001-weekly', 'weekly', 1]
    ]
  );
});
...
test('Firestore template_insert event は単一期間テンプレートだけを期間開始日の run に保存する', async () => {
  const runtime = await loadGasRuntime({
    scriptProperties: {
      ADMIN_LOGIN_ID: 'admin-login',
      ADMIN_LOGIN_PASSWORD: 'admin-password',
  ...
  const seed = createBaseDataset();
  seed.checklist_templates[0].period = 'weekly';
  ...
  assert.equal(response.statusCode, 200);
  assert.equal(response.body.result.insertedCount, 1);
...
test('Firestore template_insert event は期間混在テンプレートを拒否する', async () => {
  const runtime = await loadGasRuntime({
    scriptProperties: {
      FIRESTORE_EVENT_SYNC_SECRET: 'sync-secret'
    }
