...
const importCsvFiles = {
  ...
  notification_channels: 'docs/operations/import/notification_channels.csv',
  notification_recipients: 'docs/operations/import/notification_recipients.csv',
  notification_channel_usage: 'docs/operations/import/notification_channel_usage.csv',
...
test('bootstrapSpreadsheetTemplates は空 Spreadsheet にテンプレートを一括投入できる', async () => {
  ...
  assert.deepEqual(normalize(spreadsheet.dumpSheet('checklist_templates')), [
    ['id', 'store_id', 'name', 'period', 'notify_time', 'closing_time', 'is_active', 'created_by', 'created_at', 'updated_at'],
    ['tmpl-001', 'store-001', '日次チェックリスト', 'daily', '10:30', '00:00', 'true', 'user-mg-001', '2026-04-22T00:00:00Z', '2026-04-22T00:00:00Z']
  ]);
  assert.deepEqual(normalize(spreadsheet.dumpSheet('checklist_template_items')), [
    ['id', 'template_id', 'title', 'description', 'period', 'sort_order', 'is_required', 'is_active', 'created_at', 'updated_at'],
    ['tmpl-item-001', 'tmpl-001', '開店準備', '', 'daily', '1', 'true', 'true', '2026-04-22T00:00:00Z', '2026-04-22T00:00:00Z'],
    ['tmpl-item-002', 'tmpl-001', '清掃確認', '', 'daily', '2', 'true', 'true', '2026-04-22T00:00:00Z', '2026-04-22T00:00:00Z']
  ]);
});
