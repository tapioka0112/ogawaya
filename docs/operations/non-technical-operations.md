...
![従業員画面の今日のタスク一覧](./assets/employee-home.png)
...
![管理者画面のログイン画面](./assets/admin-login.png)
...
4. 「タグ」で `日間`、`週間`、`月間` のどれかを選ぶ。
5. 「タスクを作成」を押す。
6. 画面に「タスクを作成しました」と出ることを確認する。
...
![管理者画面のテンプレート挿入](./assets/admin-template-insert.png)
...
![管理者画面のタスク管理一覧](./assets/admin-task-management.png)
...
![従業員画面の統計カレンダー](./assets/employee-stats.png)
...
2. 画面下部の「起動時間」を見る。
3. 遅い区間名と `status` を開発者へ伝える。

起動時間ウォーターフォールのイメージです。

![従業員画面の起動時間ウォーターフォール](./assets/employee-debug-waterfall.png)
