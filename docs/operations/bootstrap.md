...
LIFF の `idToken` 検証は、`LIFF_ID` 先頭10桁の LINE Login channel ID を優先して使う。`LINE_CHANNEL_ID` は fallback として扱う。GitHub Pages 版は `pages/config.json` の `liffId` も GAS に送るが、GAS Script Properties の `LIFF_ID` がある場合はそちらを優先する。
