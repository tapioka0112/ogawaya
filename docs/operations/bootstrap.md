...
1. `gas/.clasp.example.json` を参考に、ローカルだけの `gas/.clasp.json` に `scriptId` が入っている
2. `gas/appsscript.json` に必要 scope がある
3. `npm test` が成功する

`gas/.clasp.json` は本番 Apps Script project ID を含むため、公開リポジトリへ commit しません。
...
このルールは以下のみ許可します。
- `stores/{storeId}/runs/{targetDate}/events/*` の認証済み read
